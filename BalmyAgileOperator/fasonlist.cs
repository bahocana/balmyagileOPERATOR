...
namespace BalmyAgilev1
{
    ...
    public partial class fasonlist : Form
    {
        ...
        private void LoadAltistasyonlar(string istasyontipi)
        {
            ...
            dvg_fasonlist.DataSource = result;
        }

        private void fasonlist_Load(object sender, EventArgs e)
        {
            LoadAltistasyonlar("fason");
        }

        private void fasonlist_FormClosed(object sender, FormClosedEventArgs e)
        {
            
        }

        private void dvg_fasonlist_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                // Tıklanan hücredeki değeri al
                string selectedData = dvg_fasonlist.Rows[e.RowIndex].Cells["Altisyonadi"].Value?.ToString();

                // Form1'deki TextBox'a bu değeri gönder
                _form1.settxt_Fasonisim(selectedData);
